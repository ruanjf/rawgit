...
module.exports = function (rootUrl) {
    ...
    var REGEX_GITLAB_REPO_URL = /^(.+?\/.+?)\/blob\/(.+)/i;
    ...
    return function (req, res, next) {
        ...
        if (REGEX_GITLAB_REPO_URL.test(req.path)) {
            isGitLab = true;
            var sp = config.gitlabRepo.indexOf(':');
            if (sp > 0) {
                hostname = config.gitlabRepo.substring(0, sp);
                port = parseInt(config.gitlabRepo.substring(sp+1));
            } else {
                hostname = config.gitlabRepo;
            }

            path = req.path.replace(REGEX_GITLAB_REPO_URL, '$1/raw/$2');
        }

        (isGitLab ? http : https).get({
            agent   : false,
            hostname: hostname,
            port    : port,
            path    : url.resolve(parsedUrl.path, path),
            headers : headers
...
function onResponse(req, res, next, incoming) {
    ...
    res.setHeader('Content-Type', mime.contentType(req.path, charset && charset[1]));
